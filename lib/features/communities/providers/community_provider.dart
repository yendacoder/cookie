...
class CommunityDetail extends _$CommunityDetail {
  ...
  Future<Community> build(String communityName) async {
    final response = await ref
        ...
        .get('communities/$communityName', queryParameters: {'byName': 'true'});
  ...
  Future<void> toggleJoin() async {
    ...
    try {
      ...
      // don't update the state with the API response here
      // as the API doesn't provide the full community info with rules
      // and moderators
    } catch (_) {
...
class CommunityFeedNotifier extends _$CommunityFeedNotifier {
  ...
  Future<PostFeedState> build(String communityName, String communityId) async {
    ...
    _seenIds.clear();
    final sort = await ref.watch(
      communityFeedSortProvider(communityName).future,
    );
  ...
  Future<void> loadMore() async {
    ...
    try {
      final sort =
          ref.read(communityFeedSortProvider(_communityName)).value ??
          PostSort.hot;
